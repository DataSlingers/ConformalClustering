...
means_2d = 8 * np.vstack([np.cos(np.pi * np.linspace(0.05, 0.45, K)),
                      np.sin(np.pi * np.linspace(0.03, 0.47, K))]).T

# embedding the 2D means into higher dimensions by padding with ones
means = np.ones((K, d))
means[:, :2] = means_2d

base_covariances = np.array([np.eye(d) for _ in range(K)])
base_shapes = np.zeros((K, d))
base_scales = np.zeros((K, d))
...
def run_experiment(n, var, seed):
    ...
    while not found_K:
        try:
            cc.fit('GaMM', n_components=K, max_iter=2000, n_init=25, random_state=attempt)
            found_K = True
        except ValueError:
            attempt += 1
    ...
    while not found_K:
        try:
            cc.fit('GaMMS', n_components=K, max_iter=2000, n_init=25, random_state=attempt)
            found_K = True
        except ValueError:
            attempt += 1
    ...
    while not found_K:
        try:
            gc.fit(n_components=K, max_iter=2000, n_init=25, random_state=attempt)
            found_K = True
        except ValueError:
            attempt += 1
...
if __name__ == '__main__':
    ...
    n_range = [700, 1000, 1500, 2500, 4000, 6000, 9000, 15000]
    ...
    with open(f'GaMM30D5v1_asymp.pkl', 'wb') as f:
